...
class XimiLofiCreation:
    ...
    - duruation (int/float): Output duration in seconds
    ...
    def INPUT_TYPES(cls):
        return {
            "required": {
                ...
                "duruation": (IO.FLOAT, {"default": 600.0, "min": 1.0, "max": 60*60*6, "step": 1.0}),
            }
        }
    ...
    def create_lofi(self, video_str: str, audio_str: str, duruation: float):
        if duruation <= 0:
            raise ValueError("duruation must be > 0")
        ...
        try:
            _ffmpeg_merge_loop(video_local, audio_local, float(duruation), out_path)
        except Exception:
            # Fallback to concat method
            _ffmpeg_concat_then_merge(video_local, audio_local, float(duruation), out_path)
